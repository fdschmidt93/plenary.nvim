local utils = require "plenary.utils"
...
function Component:is_under_cursor(cursor)
  ...
  local col_ok = self.position.col_end == -1 or (self.position.col_start <= col and col <= self.position.col_end)
    ...
    __index = function(tbl, name)
      ...
      if value == nil then
        value = function(value)
          local options = utils.deepcopy(tbl)
          options[name] = value
          return options
        end
      end
